...
app.post("/api/items/:id/claim", (req, res) => {
    ...
    if (!claimant)
        return res.status(400).json({ error: "Invalid claimantId" });
    const claim = {
        id: nextClaimId++,
        itemId: item.id,
...
app.post("/api/claims/:id/verify", (req, res) => {
    const claimId = Number(req.params.id);
    const { adminId, approve } = req.body;
    const admin = findUser(Number(adminId));
    if (!admin || admin.role !== "admin")
        return res.status(403).json({ error: "Only admin can verify" });
    ...
    if (item && approve)
        item.status = Models.ItemStatus.Verified;
    broadcastCounts();
    res.json(claim);
});
...
app.post("/api/items/:id/return", (req, res) => {
    ...
    const admin = findUser(Number(adminId));
    if (!admin || admin.role !== "admin")
        return res.status(403).json({ error: "Only admin can mark returned" });
    const item = items.find(i => i.id === itemId);
    if (!item)
        return res.status(404).json({ error: "Item not found" });
...
app.get("/api/counts/stream", (req, res) => {
    ...
    const onCounts = (payload) => send(payload);
...
app.post("/api/items/:id/suggest-description", (req, res) => {
    const itemId = Number(req.params.id);
    const item = items.find(i => i.id === itemId);
    if (!item)
        return res.status(404).json({ error: "Item not found" });
